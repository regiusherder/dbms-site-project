$(document).ready(function () {
    ...
    function addlabor(data) {
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("labor Added Successfully", { "status": "success" });
            table.destroy();
    ...
    function deletelabor(id) {
        ...
        swal({
            ...
            $.ajax(settings).done(function (response) {
                swal("Deleted!", "labor has been deleted.", "success");
                table.destroy();
                $('#datatable4 tbody').empty(); // empty in case the columns change
                getlabor()
            });
    ...
    function updatelabor(data, id) {
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("labor Updated Successfully", { "status": "success" });
            table.destroy();
            $('#datatable4 tbody').empty(); // empty in case the columns change
            getlabor()
        });
        ...
        $.ajax(settings).done(function (response) {
            ...
            $('.btn-edit').one("click", function (e) {
                ...
                $('#myModal').modal().one('shown.bs.modal', function (e) {
                    ...
                    $("#savethelabor").off("click").on("click", function (e) {
                        var instance = $('#detailform').parsley();
                        instance.validate()
                        console.log(instance.isValid())
                        if (instance.isValid()) {
                            jsondata = $('#detailform').serializeJSON();
                            updatelabor(jsondata, data.labor_id)
                        }
