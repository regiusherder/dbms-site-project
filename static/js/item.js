$(document).ready(function () {
    ...
    function additem(data) {
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("item Added Successfully", { "status": "success" });
            table.destroy();
    ...
    function updateitem(data, id) {
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("item Updated Successfully", { "status": "success" });
            table.destroy();
            $('#datatable4 tbody').empty(); // empty in case the columns change
            getitem()
        });
        ...
        $.ajax(settings).done(function (response) {
            ...
            $('.btn-edit').one("click", function (e) {
                ...
                $('#myModal').modal().one('shown.bs.modal', function (e) {
                    ...
                    $("#savetheitem").off("click").on("click", function (e) {
                        var instance = $('#detailform').parsley();
                        instance.validate()
                        console.log(instance.isValid())
                        if (instance.isValid()) {
                            jsondata = $('#detailform').serializeJSON();
                            updateitem(jsondata, data.item_id)
                        }
